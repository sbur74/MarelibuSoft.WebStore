...
namespace MarelibuSoft.WebStore.Common.Statics
{
	public static class FriendlyUrlHelper
	{
		...
		public static string ReplaceUmlaute(string myname)
		{
			string lower = myname.ToLower();
			if (lower.Contains("ä")) lower = lower.Replace("ä", "ae");
			if (lower.Contains("ü")) lower = lower.Replace("ü", "ue");
			if (lower.Contains("ö")) lower = lower.Replace("ö", "oe");
			if (lower.Contains("ä")) lower = lower.Replace("ß", "ss");
			lower = lower.Replace(" ", "-");
